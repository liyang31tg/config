...
-- map("c", "<c-a>", function()
-- 	vim.api.nvim_command("call setcmdpos(1)") -- 更稳定的API调用
-- 	return "" -- 关键点：必须返回空字符串
-- end, { expr = true, desc = "Cmdline Start" })
--
-- vim.keymap.set("c", "<C-A>", function()
-- 	local pos = vim.fn.getcmdpos()
-- 	-- 添加光标移动动画
-- 	for i = pos, 1, -2 do
-- 		vim.fn.setcmdpos(math.max(i, 1))
-- 		vim.cmd("redraw") -- 强制重绘
-- 		vim.defer_fn(function() end, 30) -- 30ms间隔
-- 	end
-- 	return ""
-- end, { expr = true, noremap = true, desc = "炫酷行首跳转" })

--keywordprg
map("n", "<leader>K", "<cmd>norm! K<cr>", { desc = "Keywordprg 查询文u" })
